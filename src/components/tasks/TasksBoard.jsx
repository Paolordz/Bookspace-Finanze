      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex flex-wrap gap-2 mt-2 bg-[#f8f9fc] p-3 rounded-xl border border-gray-200 min-h-[60px]">
              {userOptions.map((user) => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition flex items-center gap-2 border ${isSelected
                        ? 'bg-[#4f67eb] text-white border-[#4f67eb]'
                        : 'bg-white text-[#2a1d89] border-gray-200 hover:border-[#4f67eb]'
                      }`}
                  >
                    {isSelected ? <CheckCircle2 className="w-3.5 h-3.5" /> : <div className="w-3.5 h-3.5 rounded-full border border-gray-300" />}
                    {getAssigneeLabel(user.uid)}
                  </button>
                );
              })}
              {userOptions.length === 0 && (
                <p className="text-xs text-[#b7bac3]">No hay usuarios disponibles.</p>
              )}
            </div>
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    ...
                                    const newAssignees = isAssigned
                                      ...
                                      : uniqueValues([...current, user.uid]);
                                    updateTask(task.id, { assignees: newAssignees });
                                  }}
                                  className={`px-2 py-1 rounded-lg text-xs font-medium border transition ${isAssigned
                                      ? 'bg-[#4f67eb]/10 text-[#4f67eb] border-[#4f67eb]/20'
                                      : 'bg-white text-gray-500 border-gray-200 hover:border-[#4f67eb]/50'
                                    }`}
                                >
